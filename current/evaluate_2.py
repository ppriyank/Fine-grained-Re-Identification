...
def test_rerank2(model, queryloader, galleryloader, pool, use_gpu):
    ...
    with torch.no_grad():
        ...
        gf2 = gf.numpy()
        qf2 = qf.numpy()
        gf2=gf2.transpose()/np.power(np.sum(np.power(gf2,2),axis=1),0.5)
        gf2=gf2.transpose()
        qf2=qf2.transpose()/np.power(np.sum(np.power(qf2,2),axis=1),0.5)
        qf2=qf2.transpose()
        score = -np.matmul(qf2 , gf2.transpose())
...
# path= "/scratch/pp1953/resnet_" + args.opt + "/" + "*.pth.tar"
# path= "/beegfs/pp1953/resnet/trained/ResNet50ta_bt15_" + "*.pth.tar"
# path= "/scratch/pp1953/resnet_" + args.opt + "/"
dataset = args.dataset

for folder in  os.listdir("/scratch/pp1953/"):
    if len(folder.split("_")) ==3:
        print("==========" , folder)
        if dataset == "mars":
            if folder.split("_")[-1] != '53':
                continue 
        else:
            if folder.split("_")[-1] == '53':
                continue
        path= "/scratch/pp1953/"+ folder+ "/" + "*.pth.tar"
        for file in glob.glob(path):
            ...
            print("LOADED :D :D :D ")
            test_rerank2(model, queryloader, galleryloader, args.pool, use_gpu)
            test_rerank1(model, queryloader, galleryloader, args.pool, use_gpu)
            


    
# python evaluate_2.py -d=mars -a="ResNet50ta_bt15" --opt=1_53 --height=200 --width=150 