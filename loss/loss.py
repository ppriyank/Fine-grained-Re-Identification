...
class Satisfied_Rank_loss(nn.Module):
    
    def __init__(self, mr=2, ms=1, mcs= 0.005, use_gpu=True , b_l1=2, b_h1=20 , b_l=1, b_h=10) :
        super(Satisfied_Rank_loss, self).__init__()
        self.mr = mr
        self.ms = ms
        self.use_gpu=use_gpu
        self.mcs =  mcs
    ...
    def forward(self, y1, y2, label, T_s, T_b ):
        
        targets = torch.zeros(y1.size()).scatter_(1, label.unsqueeze(1).data.cpu(), 1)
        if self.use_gpu: targets = targets.to(torch.device('cuda'))

        y1 = y1.softmax(1)
        y2 = y2.softmax(1)

        Dkl = y1 * torch.log( y1 / (y2 + 1e-6) + 1e-6)
        Dkl = Dkl.sum(1).mean()
        lcns = Dkl - self.mcs
        ...
        # lr = torch.relu(y1 - y2 + 1)
        lr = torch.relu(y1 - y2 + self.mr)
        ls = torch.relu(-y1 - y2 + 2 * self.ms)

        lsr = torch.min(lr, ls)
        lsr = lsr.mean()

        
        l_low = torch.relu(self.b_l1 * self.h * self.w - T_s) 
        l_height = torch.relu(T_s - self.b_h1 * self.h * self.w ) 
        
        l_sm = (l_low + l_height) / (self.h * self.w )
        l_sl = l_sm.mean()
        
        l_low = torch.relu(self.b_l * self.h * self.w - T_b) 
        l_height = torch.relu(T_b - self.b_h * self.h * self.w ) 
        
        l_sm = (l_low + l_height) / (self.h * self.w )
        l_sm = l_sm.mean()
        

        return lsr , lcns ,l_sm , l_sl



class Satisfied_Rank_loss2(nn.Module):
    
    def __init__(self, mr=2, ms=1, mcs= 0.005, use_gpu=True , b_l1=1, b_h1=20) :
        super(Satisfied_Rank_loss2, self).__init__()
        self.mr = mr
        self.ms = ms
        self.use_gpu=use_gpu
        self.mcs =  mcs
    ...
    def forward(self, y1, y2, label , T ):
        ...
        Dkl = y1 * torch.log( y1 / (y2 + 1e-12) + 1e-12)
        Dkl = Dkl.sum(1).mean()
        lcns = torch.relu(Dkl - self.mcs)
        ...
        if type(T) == torch.Tensor:
            l_low = torch.relu(self.b_l1 * self.h * self.w - T) 
            l_height = torch.relu(T - self.b_h1 * self.h * self.w ) 
            
            l_sm = (l_low + l_height) / (self.h * self.w )
            l_sl = l_sm.mean()
        
            return lsr , lcns  , l_sl
        else:
            return lsr , lcns  
